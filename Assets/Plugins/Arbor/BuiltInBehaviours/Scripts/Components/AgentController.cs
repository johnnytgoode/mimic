...
namespace Arbor
{
	...
	[BuiltInComponent]
	[HelpURL(ArborReferenceUtility.docUrl + "manual/builtin/agentcontroller.html")]
	...
	public sealed class AgentController : MovingEntity, ISerializationCallbackReceiver
	{
...
#if ARBOR_DOC_JA
		/// <summary>
		/// 移動先地点に対しSample Position Distanceの範囲内にあるNavMesh上の位置を見つけるフラグ。
		/// </summary>
#else
		/// <summary>
		/// Flag to find a position on the NavMesh that is within the Sample Position Distance to the destination.
		/// </summary>
#endif
		[SerializeField]
		private bool _SamplePositionEnabled = false;
...
#else
		/// <summary>
		/// Distance to perform SamplePosition
		/// </summary>
#endif
		[SerializeField]
		private float _SamplePositionDistance = 0.1f;
		...
		/// </summary>
#endif
		public bool samplePositionEnabled
		{
			get
			{
				return _SamplePositionEnabled;
			}
			set
			{
				_SamplePositionEnabled = value;
			}
		}
		...
		/// </summary>
#endif
		public float samplePositionDistance
		{
			get
			{
				return _SamplePositionDistance;
			}
			set
			{
				_SamplePositionDistance = value;
			}
		}
		...
		bool BeginMoveTo(Vector3 toPos)
		{
			...
			if (_SamplePositionEnabled)
			{
				if (NavMesh.SamplePosition(toPos, out var hit, _SamplePositionDistance, _Agent.areaMask))
				{
					toPos = hit.position;
				}
				else
								...
								case OffMeshLinkType.LinkTypeManual:
									{
										...
										{
											offMeshLinkGameObject = component.gameObject;
										}

										if (offMeshLinkGameObject != null && offMeshLinkGameObject.TryGetComponent<OffMeshLinkSettings>(out var offMeshLinkController))
										{
